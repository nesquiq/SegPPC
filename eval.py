...
def writelog(filepath, best_th, best, metric, comment):
    filepath = filepath
    logfile = open(filepath, 'a')
    import time
    logfile.write(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
    logfile.write('\t%s\n' % comment)
    logfile.write('\tbest bg_threshold: %s\n' % str(best_th))
    logfile.write('\tbest mIouU: %s\n' % str(best))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--list", default='/home/subin/Datasets/VOC2012/VOCdevkit/VOC2012/ImageSets/Segmentation/train.txt', type=str)
    # parser.add_argument("--predict_dir", default='./out_rw', type=str)
    parser.add_argument("--predict_dir", default='/home/subin/Datasets/cluster_proto_v4_cam_npy', type=str)
    parser.add_argument("--gt_dir", default='/home/subin/Datasets/VOC2012/VOCdevkit/VOC2012/SegmentationClass', type=str)
    parser.add_argument('--logfile', default='./evallog_cluster_proto_v4_1.txt', type=str)
    ...
    if not args.curve:
        ...
        for i in range(30):
            ...
            if best < loglist['mIoU']:
                best = loglist['mIoU']
                best_th = i
            print('%d/30 background score: %.3f\tmIoU: %.3f%%' % (i, t, loglist['mIoU']))
        writelog(args.logfile, best_th, best, {'mIoU': l}, args.comment)
