...
class Net(network.resnet38d.Net):
    def __init__(self):
        ...
        def forward(self, x):
            ...
            f_all_res = torch.cat([x_s2, f8_0, f8_1, f8_3, f8_4], dim=1)

            f_all_res = F.relu(self.fc_proj_256(f_all_res))
            cam = F.interpolate(cam, (H, W), mode='bilinear', align_corners=True)
            # refined_cam, cluster_labels, prototype_256, prototype_4096 = self.Cluster_module(f_all_res, f_4096)
            """
            ...
            return cam, cam_rv, f_proj, cam_rv_down, f_all_res, fea

    """
    TODO: define clustering functions with Fast_mean_shift_EUC
    """

    def Cluster_module(self, f_all_res, f_4096):
        with torch.no_grad():
            ...
            nn,cc,hh,ww = f_4096.size()
            f_4096 = f_4096.detach().permute(1, 0, 2, 3) # c, n, h, w
            f_4096 = f_4096.reshape(cc,nn*hh*ww) # c, nhw
            f_4096 = f_4096.permute(1,0) # nhw, c

            cluster_labels, cluster_centers = Clustering(f_all_res)

            prototype_256 = torch.zeros((len(cluster_centers)),256)
            prototype_4096 = torch.zeros((len(cluster_centers)),4096)
            for i, c in enumerate(torch.unique(cluster_labels)):
                prototype_256[i] = f_all_res[torch.where(cluster_labels==c)].mean()
                prototype_4096[i] = f_4096[torch.where(cluster_labels==c)].mean()

            cluster_pixel_corr_map = f_all_res * cluster_centers.T

            prototype_classifications = self.fc8(prototype_4096.cuda())
        
            refined_cam = cluster_pixel_corr_map.cuda() * prototype_classifications

        return refined_cam, cluster_labels, prototype_256, prototype_4096
