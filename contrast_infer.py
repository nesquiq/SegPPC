...
if __name__ == '__main__':
    ...
    # parser.add_argument("--infer_list", default="voc12/train_sub.txt", type=str)
    ...
    parser.add_argument("--local_clustering", action="store_true", default=False)

    args = parser.parse_args()
    model = getattr(importlib.import_module(args.network), 'Net')()
    model.load_state_dict(torch.load(args.weights, map_location="cuda:0"))
    out_cam_pred_alpha_str = '_' + str(args.out_cam_pred_alpha).split('.')[-1]
    model.eval()
    model.cuda()
    ...
    for iter, (img_name, img_list, label) in tqdm(enumerate(infer_data_loader), total=len(infer_data_loader)):
        ...
        def _work(i, img):
            with torch.no_grad():
                with torch.cuda.device(i % n_gpus):
                    if args.local_clustering:
                        _, cam, _, _, _, _ = model_replicas[i % n_gpus](img.cuda())
                    else:
        ...
        if args.out_cam_pred is not None:
            ...
            if not os.path.exists(args.out_cam_pred+out_cam_pred_alpha_str):
                os.makedirs(args.out_cam_pred+out_cam_pred_alpha_str)

            bg_score = [np.ones_like(norm_cam[0]) * args.out_cam_pred_alpha]
            pred = np.argmax(np.concatenate((bg_score, norm_cam)), 0)
            # pred = pred/(pred.max()+1e-6)
            # pred*=255
            imageio.imsave(os.path.join(args.out_cam_pred+out_cam_pred_alpha_str, img_name + '.png'), pred.astype(np.uint8))
        ...
        if args.out_crf is not None:
            crf_pred = _crf(cam_dict, args.out_cam_pred_alpha)
            folder = args.out_crf + out_cam_pred_alpha_str
