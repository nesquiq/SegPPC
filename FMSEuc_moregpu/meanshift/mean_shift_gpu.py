...
from FMSEuc_moregpu.meanshift.batch_seed import meanshift_torch
...
def mean_shift_euc(X_gpu, bandwidth=None, seeds=None, 
    ...
    """
    init_bw = bandwidth.item()
    if bandwidth is None:
        # bandwidth = estimate_bandwidth(X)
        bandwidth = estimate_bw(X_gpu)
    elif bandwidth <= 0:
    ...
    labels_torch = torch.tensor([]).cuda()
    labels_torch = labels_torch.reshape(0, 256)
    number_torch = torch.tensor([]).cuda()
    number_torch = number_torch.reshape(0, 1)
    # bandwidth = bandwidth.cuda()
    t1 = time()
    global SEED_NUM
    if GPU == True:
        #GPU ver
        ...
        while True:
            ...
            labels, number = meanshift_torch(X_gpu, seeds, bandwidth)#gpu calculation
            ...
            labels_torch = torch.cat([labels_torch,labels],0)
            # labels_torch = torch.cat([labels_torch,torch.tensor(labels)],0)
            number_torch = torch.cat([number_torch,torch.tensor(number).cuda().reshape(-1,1)],0)
            tor = torch.concat([number_torch,labels_torch],-1)
            _, id1 = torch.sort(tor[:,0], descending=True)
            tor_sort = tor[id1]
            _, id2 = torch.sort(tor_sort[:,1], descending=True, stable=True)
            tor_sort = tor_sort[id2]
            sorted_centers = tor_sort[:,1:]

            ## 마스크로 사용예정
            # unique = np.ones(len(sorted_centers), dtype=np.bool)
            unique_torch = torch.ones(sorted_centers.shape[0], dtype=torch.bool)
            ...
            for i, center in enumerate(sorted_centers):
                if unique_torch[i]:
                    ...
                    ## 해당 idx의 마스크 0
                    _, neighbor_idxs = rad_neighbor_cosine(sorted_centers,sorted_centers[i].unsqueeze(0),5,bandwidth)
                    unique_torch[neighbor_idxs] = 0
                    ## 현재 포인트 마스크 1
                    unique_torch[i] = 1 # leave the current point as unique
            cluster_centers = sorted_centers[unique_torch]
            clst_centers = sorted_centers[unique_torch]

            ## 남은 센터들에 1 nn으로 모든 점들 레이블링
            # assign labels
            # nbrs = NearestNeighbors(n_neighbors=1).fit(cluster_centers)
            # out_labels = np.zeros(n_samples, dtype=np.int)
            distances, idxs = KNN(clst_centers.cuda(), X_gpu, 1)
            # bw 밖의 orphan을 버릴 것인가 아니면 그래도 nn으로 가져갈 것인가
            if cluster_all:
                out_labels_torch = idxs.flatten()
            else:
                ## not implemented for torch tensor
                pass
                # out_labels.fill(-1)
                # bool_selector = distances.flatten() <= bandwidth
                # out_labels[bool_selector] = idxs.flatten()[bool_selector]
            
            bg_num = torch.sum(out_labels_torch==0)
            # r = 1-bg_num/out_labels_torch.size().item()
            #seed number adjust
        ...
        # fin_bw = bandwidth.item()
        # wandb.log({'iter_count': count,
        #             'initial_bw': init_bw,
        #             'final_bw': fin_bw,
        #             'fin_over_init': fin_bw/init_bw,
        #             'clustering_time': elapsed})

        return cluster_centers, out_labels_torch
...
class MeanShiftEuc(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X_gpu, y=None):
        ...
        self.cluster_centers_, self.labels_ = \
            mean_shift_euc(X_gpu, bandwidth=self.bandwidth, seeds=self.seeds,
